...
async function loadLostPets() {
  try {
    const res = await authFetch('/lost/');
    const data = await res.json();
...
async function loadFoundPets() {
  try {
    const res = await authFetch('/found/');
    const data = await res.json();
...
async function handleEditFormSubmit(e) {
  ...
  try {
    const res = await authFetch(`/pet/${petId}/edit/`, {
      method: 'PUT',
      body: JSON.stringify(data)
    });
    const result = await res.json();
    if (!res.ok) throw new Error(result.error || 'Failed to update pet.');
...
async function deletePet(id) {
  ...
  document.getElementById('confirmYes').onclick = async () => {
    try {
      const res = await authFetch(`/pet/${id}/delete/`, {
        method: 'DELETE'
      });
      const data = await res.json();
      if (res.ok) {
...
function confirmDeleteAll(status) {
  ...
  document.getElementById('confirmYes').onclick = async () => {
    try {
      const res = await authFetch(`/${status}/delete_all/`, {
        method: 'DELETE',
        headers: {
          'Content-Type': 'application/json',
...
async function markAsFound(id) {
  closeModal('petModal');
  try {
    const res = await authFetch(`/pet/${id}/mark_found/`, { 
      method: 'POST',
      headers: {
        'Content-Type': 'application/json',
        'X-CSRFToken': getCsrfToken()
...
const modalsToWatch = ['dashboardInfoModal', 'lostInfoModal', 'foundInfoModal'];
